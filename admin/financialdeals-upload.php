...
$meName = $_POST["txt-fd-name"];
$meDesc = $_POST["txt-fd-desc"];

if(isset($_FILES["file-fd-img"]) && $_FILES["file-fd-img"]["error"] == UPLOAD_ERR_OK && isset($_FILES["file-fd-pdf"]) && $_FILES["file-fd-pdf"]["error"] == UPLOAD_ERR_OK) {

	// set the upload directory here
	$UploadDirectoryImg	= '../content-upload/FinancialDealsImg/'; //specify upload directory ends with / (slash)

	$UploadDirectoryPdf	= '../content-upload/FinancialDealsPdf/'; //specify upload directory ends with / (slash)

	//Is file size is less than allowed size.
	if ($_FILES["file-fd-img"]["size"] > 5242880) {
		die("Img File size is too big!");
	}

	//Is file size is less than allowed size.
	if ($_FILES["file-fd-pdf"]["size"] > 5242880) {
		die("Pdf File size is too big!");
	}

	$FileNameImg          	= strtolower($_FILES['file-fd-img']['name']);
	$FileExtImg           	= substr($FileNameImg, strrpos($FileNameImg, '.')); //get file extention

	$FileNamePdf          	= strtolower($_FILES['file-fd-pdf']['name']);
	...
	if(move_uploaded_file($_FILES['file-fd-img']['tmp_name'], $UploadDirectoryImg.$NewFileNameImg )) {   // img upload successful.
		if(move_uploaded_file($_FILES['file-fd-pdf']['tmp_name'], $UploadDirectoryPdf.$NewFileNamePdf)) {   // pdf upload success.
			$result = RegisterTopic($meName, $meDesc, $UploadDirectoryImg.$NewFileNameImg, $UploadDirectoryPdf.$NewFileNamePdf, "FinancialDeals");
			if($result[0] == "-1") {
				...
				header("Location: index.php?fd=1");	
			}
		}
		else {
